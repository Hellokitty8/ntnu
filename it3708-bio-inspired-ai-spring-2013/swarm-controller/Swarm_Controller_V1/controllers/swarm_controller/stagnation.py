...
class stagnation(object):
    ...
    def evaluate_pushing(self, dist_value, prev_dist_value):
        ...
        if abs(dist_diff7) > DISTANCE_DIFF_TRESHOLD and abs(dist_diff0) > DISTANCE_DIFF_TRESHOLD:
            self.has_recovered = True
            self.green_LED_state = OFF
            self.align_counter = 0
        elif dist_value[5] > NEIGHBOR_LIMIT and dist_value[2] > NEIGHBOR_LIMIT: # Has Neighbors
            self.has_recovered = True
            self.green_LED_state = OFF
            self.align_counter = 0
        elif dist_value[5] > NEIGHBOR_LIMIT or dist_value[2] > NEIGHBOR_LIMIT: # Has Neighbors
