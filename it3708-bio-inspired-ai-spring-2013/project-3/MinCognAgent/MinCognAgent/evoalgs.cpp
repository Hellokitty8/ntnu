	...
	//while(++runs <= maxruns){
		...
		while(generationCount < generations && !solutionFound){
			evolveChildren();		//Evolve current children
				...
				//fitFunctions.visualizeCurrentBest();
		...
		// for printing generation of best solution found (if any)
		ofstream phenoms;
		phenoms.open ("phenoms.txt", fstream::in | fstream::out | fstream::app);
		phenoms << runs << " " << generationCount << " " << globalBestFitness << " ";
		phenoms.close();

		myfile.close();
		fitFunctions.visualizeCurrentBest();
		fitFunctions.writeBestPhenomToFile();
		statFunctions.writeParametersToFile(populationSize, generations, crossoverRate, crossoverPoints, mutationPercentage, mutation, elitism, generationCount, selFunctions.returnProtocol(), selFunctions.returnMechanism());
		initialRun = false;
		do{
...
void evoalgs::setEAParameters(){
	...
	genomSize = 34;//eaHelper.readInt("Genom Size", MIN_GENOM, MAX_GENOM);
	generations = 100;//eaHelper.readInt("Evolution Cycles", MIN_GENERATIONS, MAX_GENERATIONS);
	populationSize = 200;//eaHelper.readInt("Population Size", MIN_POPULATION, MAX_POPULATION);
	...
	elitism = 1; //eaHelper.readInt("Elitism", MIN_ELITISM, MAX_ELITISM);
...
void evoalgs::reproduction(){
	...
	int r1,	r2,	i, p = parents.size(), mutationBonus;
	float random;
	individual* c;
	...
	for(i = elitism; i < youths.size(); i++)  {				
		...
		if(random < mutationPercentage){
			...
			//mutation = mutation*mutationBonus;
			youths[i]->mutateGenom(mutation);
		}
	}
}
...
void evoalgs::crossover(individual* a, individual* b, int numberOfChildren){
	...
	individual* c;
	vector<Gene> newGenom;
	vector<vector<Gene> > parents;
