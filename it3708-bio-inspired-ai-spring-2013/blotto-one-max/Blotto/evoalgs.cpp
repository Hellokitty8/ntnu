...
void evoalgs::evolutionCycle(){
	...
	while(command != 'n'){
		...
		myBlottoFile.open("maxFitnessIndividual.txt");
		cout << "\nRunning the evolutionary algorithm!";
		while(generationCount < generations && !solutionFound){
			//Generate phenotypes
			evolveChildren();	
			//Test fitness of the current generation
			fitFunctions.testFitness(youths);						 							
			stats = statFunctions.calculateGenerationStats(youths);	
			myfile << generationCount+1 << " " << stats[0] << " " << stats[1] << " " << stats[2] << " " << stats[3] << endl;
			if(stats[0] >= treshold && treshold != 0)									
				solutionFound = true;
			//Select the genes to be passed on
			selFunctions.adultAndParentSelection(youths, adults, parents, populationSize, elitism);
			if ((generationCount+10) %10 == 0)
				eaHelper.printPhenom(parents[0], myBlottoFile);
...
//Crossover function, takes 2x individuals and the number of crossover poitns
//Able to handle different segment sizes. Feks: 2x crossover on 20 bit = 6 7 7 bits
void evoalgs::crossover(individual* a, individual* b, int numberOfChildren){

	individual* c;
	vector<int> newGenom;
	vector<vector<int> > parents;
	int variableSize = a->getGenomcode().size(); // this is to make sure the size of the genomcode is the same here as in the genom, without having to change it in more than one place

	int segmentSize = (variableSize)/(crossoverPoints+1);
	int diff = (variableSize)-(segmentSize*(crossoverPoints+1));
...
//Crossover with a randomly selected crossover point
void evoalgs::randomCrossover(individual* a, individual* b, int numberOfChildren){

	int variableSize = a->getGenomcode().size(); 
	int r = rand()%variableSize, p;
	individual* c;
	vector<int> newGenom;
	vector<vector<int> > parents;
	parents.push_back(a->getGenomcode());
