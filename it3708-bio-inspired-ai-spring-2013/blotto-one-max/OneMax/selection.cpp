...
//Full generational replacement function
...
void selection::generationMixing(vector<individual*> &juveniles, vector<individual*> &adults, int count, int elitism){
	...
	adults.insert(adults.end(), juveniles.begin(), juveniles.end());	//Add juviniles to adult vector
	sort(adults.begin(), adults.end(), fitnessSortFunc);				//Sort after fitness value
	for(it = adults.begin()+(count+elitism); it != adults.end();){		//Remove the unfit ones
...
//Tournament selection function (Deterministic!)
void selection::tournamentSelection(vector<individual*> &adults, vector<individual*> &parents, int count, int elitism){
	...
	for(int i = 0; i < count; i++){
		...
		if(r < e){
			parents.push_back(tournament[0]);							 //Winner = fittest individual in tournament
		}
		else{
			parents.push_back(tournament[1]);
		}
		tournament.clear();
	}
