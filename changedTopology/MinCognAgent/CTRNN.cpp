...
//Changed the equation to be equal to the one presentet in the project
//Description, the original code is commented out
void CTRNN::EulerStep(double stepsize)
{
  ...
  for (int i = 1; i <= size; i++) {
	...
	states[i] += Rtaus[i] * (input - states[i] + biases[i]);
  }
  // Update the outputs of all neurons.
  for (int i = 1; i <= size; i++)
	  outputs[i] = sigmoid(gains[i] * states[i]);
